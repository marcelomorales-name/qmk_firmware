...
def parse_positions(keyboard_json: Path, layout_name: str) -> "list[list[float]]":
    doc = json.loads(keyboard_json.read_text())
    try:
        layout = doc["layouts"][layout_name]["layout"]
    except KeyError:
        sys.exit(f"error: no layouts.{layout_name}.layout array in {keyboard_json}")
    positions = []
    for entry in layout:
        x, y = entry["x"], entry["y"]
...
def render(keymap_c: Path, keyboard_json: Path, layout_name: str, out_path: Path) -> None:
    layers = parse_keymaps(keymap_c)
    order = parse_layer_order(keymap_c)
    positions = parse_positions(keyboard_json, layout_name)
...
def main() -> None:
    ...
    ap.add_argument("--layout-name", default=DEFAULT_LAYOUT_NAME)
    ap.add_argument("--out", type=Path, default=DEFAULT_OUT)
    args = ap.parse_args()
    render(args.keymap_c, args.keyboard_json, args.layout_name, args.out)


TEMPLATE = r"""<title>Corne V4 Marce Layout</title>
...
<div class="wrap">
  <header>
    <div>
      <h1>Corne<span> V4</span> &middot; marce keymap</h1>
      <p class="sub">Five layers, one 42&#8209;key split &mdash; rendered straight from the firmware source, key positions pulled from the board's own <code>info.json</code>.</p>
    </div>
    <div class="path">keyboards/crkbd/keymaps/marce/keymap.c</div>
  </header>
  ...
  <div class="notes">
    ...
    <div class="note">
      <h3>Dual&#8209;role keys</h3>
      <p><code>Esc</code>/<code>Gui</code>, <code>Quote</code>/<code>Gui</code>, and <code>Z</code>/<code>Slash</code> with <code>Ctrl</code> are mod&#8209;taps. The right thumb key is a tap&#8209;dance: tap for <code>Space</code>, double&#8209;tap for <code>Enter</code>.</p>
    </div>
  </div>
...
try { active = localStorage.getItem("crkbd-active-layer") || active; } catch(e) {}
...
function buildTabs(){
  ...
  Object.keys(LAYER_META).forEach(key => {
    ...
    b.addEventListener("click", () => { active = key; try{localStorage.setItem("crkbd-active-layer", key);}catch(e){} render(); });
    nav.appendChild(b);
  });
}
